import { Post, Body, Patch, Controller, Get, UseGuards, Req, Res, HttpStatus } from '@nestjs/common';
...
const teste = new User({ nickname: 'oi', token: null, validCode: true, userId:"",
email:"",
username:"", tfaSecret: ""});
 ...
 //   }
  ...
  //      return res.redirect(process.env.HOST_FRONT);
    ...
    async verifyTfaSecret(@Req() req , @Body() body): Promise<void>{
      ...
      return this.authService.verifyTfaSecret(await this.userService.getUser('acosta-a') ,body.code)//verificar com user
    }
}
